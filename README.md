...
```bash
bin/rails g oscar:audit:install
bin/rails db:migrate
```

What the installer does:

- Copies the engine's migrations into your app (via `railties:install:migrations FROM=oscar_audit`).
- Creates `app/models/application_log.rb`, an abstract base that subclasses `Oscar::Audit::ApplicationLog`. Define your app-specific audit handlers by subclassing `ApplicationLog`.
- Adds `spec/support/oscar_audit.rb` (for RSpec), which disables application logs by default in tests and re-enables them for feature specs and examples tagged with `require_application_logs`.

Note: If the gem ships new migrations in the future, re-run the installer or run:

```bash
bin/rails railties:install:migrations FROM=oscar_audit
```
...
ApplicationLog (generated by the installer; it subclasses `Oscar::Audit::ApplicationLog`). This model
persists your app-specific details and, after it is created, automatically creates the associated audit Log.
